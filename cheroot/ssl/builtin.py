...
class BuiltinSSLAdapter(Adapter):
    ...
    CERT_KEY_TO_LDAP_CODE = {
        'countryName': 'C',
        'stateOrProvinceName': 'ST',  # or 'SP' (SSLeay)
        'localityName': 'L',
        'organizationName': 'O',
    ...
    def __init__(
        ...
        if self._server_env:
            ...
            if all(x in cert for x in {ssl.PEM_HEADER, ssl.PEM_FOOTER}):
                cert = cert.split(ssl.PEM_HEADER, 1)[-1]
                cert = cert.split(ssl.PEM_FOOTER, 1)[0]
                cert = ssl.PEM_HEADER + cert + ssl.PEM_FOOTER
                self._server_env['SSL_SERVER_CERT'] = cert
    ...
    def wrap(self, sock):
        ...
        # Python 3.7+
        # set the SNU callback here incase the user supplies their own
        # but don't override their callback
        if (
            hasattr(self.context, 'sni_callback') and ssl.HAS_SNI
            and self.context.sni_callback is None
        ):
    ...
    def get_environ(self, sock):
        ...
        ssl_environ = {
            ...
            'SSL_VERSION_INTERFACE': 'Python %s' % sys.version,
        ...
        if hasattr(sock.context, 'get_ciphers'):
            for cip in sock.context.get_ciphers():
                if cip['name'] == cipher[0] and cip['protocol'] == cipher[1]:
                    ssl_environ['SSL_CIPHER_ALGKEYSIZE'] = cip['alg_bits']
                    break

        # Python 3.7+ sni_callback
        if hasattr(sock, 'sni') and sock.sni:
    ...
    def env_cert_dict(self, env_prefix, parsed_cert):
        ...
        for cert_key, env_var in self.CERT_KEY_TO_ENV.items():
            ...
            if env_var == 'SAN':
                env.update(self.env_san_dict(key, value))
            elif env_var.endswith('_DN'):
                env.update(self.env_dn_dict(key, value))
            else:
                env[key] = str(value)
    ...
    def env_san_dict(self, env_prefix, cert_value):
