...
class WSGIEnvironmentGateway(Gateway_10):
    """Gateway responding with the WSGI Environment."""

    def respond(self):
        """Respond with WSGI environment via HTTP."""
        # drop files so that it can be pickled
        ...
        if req_uri == '/':
            req.status = b'200 OK'
            req.ensure_headers_sent()
            req.write(pickle.dumps(env))
            return
        return super(WSGIEnvironmentGateway, self).respond()


def make_tls_http_server(
    bind_addr, ssl_adapter, request,
...
def tls_http_server(request):
    """Provision an HTTP server creator as a fixture."""
    return functools.partial(make_tls_http_server, request=request)


@pytest.fixture
def tls_wsgi_server(request):
    """Provision a WSGI server creator as a fixture."""
    return functools.partial(
        make_tls_http_server, request=request,
        gateway=WSGIEnvironmentGateway,
    )
...
def test_ssl_env(
    ...
    with client_cert.private_key_and_cert_chain_pem.tempfile() as cl_pem:
        ...
        tlswsgiserver = tls_wsgi_server((interface, port), tls_adapter)
