...
fails_under_py3 = pytest.mark.xfail(six.PY3, reason='Fails under Python 3')
...
@pytest.mark.parametrize(
    'adapter_type',
    ('builtin', pytest.param('pyopenssl', marks=fails_under_py3)),
)
        ...
        with cert.cert_chain_pems[0].tempfile() as cert_temp_path:
            ...
            if adapter_type == 'builtin':
                ...
                with patch('ssl.SSLContext.load_cert_chain', MagicMock):
                    tls_adapter = tls_adapter_cls(cert_temp_path, ca_temp_path)
            else:
                tls_adapter = tls_adapter_cls(cert_temp_path, ca_temp_path)
