...
TEST_CASES = (
    ...
    *[
    ...
    # Case 2: Unknown unacceptable OSError (Should return False)
    (UNACCEPTABLE_ERR_NUM, 'Unknown OSError', OSError, False),
)

# Generate test IDs
TEST_IDS = (
    # IDs for acceptable errors
    *[f'acceptable-{err_name}' for _, err_name in ACCEPTABLE_SHUTDOWN_ERRORS],
    # ID for unacceptable error
...
def test_socket_shutdown_error_robustness(
    ...
    We test this function across various exception types and error codes,
    including success and failure paths.
    """
    # 1. Instantiate the error
    assert exc_class is OSError, (
        f'Invalid exception class for test case: {exc_class}'
    )
